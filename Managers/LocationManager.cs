...
public class Location
{
    ...
    {
        foreach (ZNetView netView in registeredLocations.SelectMany(l =>
                     l.location.GetComponentsInChildren<ZNetView>(true)))
        {
            if (__instance.m_namedPrefabs.ContainsKey(netView.GetPrefabName().GetStableHashCode()))
            {
                string otherName =
                    Utils.GetPrefabName(__instance.m_namedPrefabs[netView.GetPrefabName().GetStableHashCode()]);
        ...
        foreach (Location location in registeredLocations)
        {
            if (location.MapIconSprite is { } icon)
            {
                __instance.m_locationIcons.Add(new Minimap.LocationSpriteData
                    { m_icon = icon, m_name = location.location.name });
            }
        }
    }
    ...
    internal static void Patch_FejdStartup()
    {
        if (ConfigurationEnabled && firstStartup)
        {
            ...
            foreach (Location location in registeredLocations)
            {
                ...
                {
                    ConfigEntry<string> spawnerCreature = config(location.location.name, $"{kv.Key} spawns", kv.Value,
                    ...
                    spawnerCreature.SettingChanged += (_, _) =>
                    {
                        ...
                        if (ZNetScene.instance &&
                            location.location.transform.GetComponentsInChildren<CreatureSpawner>()
